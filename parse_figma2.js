...
allNodes.forEach(n => {
  ...
  if (n.content === 'Jarvis by Snam' || n.content === 'Titolo slide' || n.content === 'Titolo secondario' || n.content === 'Grazie!') {
    if (titleIdx < titles.length) {
      code += `await figma.modify("${n.id}", { characters: "${titles[titleIdx].t}" }); // was: ${n.content}\n`;
      titleIdx++;
    }
  }
});
...
let subIdx = 0;
allNodes.forEach(n => {
  if (n.content === 'Framework UX' || n.content === 'Sottotitolo slide' || n.content.includes('punto 1\\npunto 2') || n.content.includes('punto 1\npunto 2')) {
    if (subIdx < titles.length) {
       code += `await figma.modify("${n.id}", { characters: "${titles[subIdx].s}" }); // was: ${n.content.replace(/\n/g, '\\n')}\n`;
       subIdx++;
    }
  }
});
